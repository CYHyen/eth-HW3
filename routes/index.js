...
router.get('/allBalance', async function (req, res, next) {
  ...
  let coinBalance = await bank.methods.getBankBalance().call({ from: req.query.account })
  res.send({
    ethBalance: ethBalance,
    bankBalance: bankBalance,
    coinkBalance: coinBalance
  })
});
...
router.post('/mintCoin', function (req, res, next) {
  let bank = new web3.eth.Contract(contract.abi);
  bank.options.address = req.body.address;
  bank.methods.mintNCCUcoin(req.body.value).send({
    from: req.body.account,
    gas: 3400000
  })
    .on('receipt', function (receipt) {
...
router.post('/buyCoin', function (req, res, next) {
  let bank = new web3.eth.Contract(contract.abi);
  bank.options.address = req.body.address;
  bank.methods.buyNCCUcoin(req.body.value).send({
    from: req.body.account,
    gas: 3400000
  })
    .on('receipt', function (receipt) {
...
router.post('/transferCoin', function (req, res, next) {
  let bank = new web3.eth.Contract(contract.abi);
  bank.options.address = req.body.address;
  bank.methods.transferNCCUcoin(req.body.to, req.body.value).send({
    from: req.body.account,
    gas: 3400000
  })
    .on('receipt', function (receipt) {
...
router.post('/transferTo', async function (req, res, next) {
  ...
  web3.eth.getGasPrice().then(gasPrice => {
    ...
    let gas_gasprice = gas * gasPrice;
    let exact_number = transferAmount - gas_gasprice;
