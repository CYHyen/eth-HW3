...
let mintNCCUcoin = $('#mintNCCUcoin');//mintNCCUcoin
let mintNCCUcoinButton = $('#mintNCCUcoinoinButton');
let buyNCCUcoin = $('#buyNCCUcoin');//buyNCCUcoin
let buyNCCUcoinButton = $('#buyNCCUcoinButton');
let transferNCCUcoinTo = $('#transferNCCUcoinTo');//trainsferNCCUcoin
...
update.on('click', function () {
	if (bankAddress != "") {
		...
		$.get('/balance', {
			...
			$('#nccucoinBalance').text('銀NCCU COIN餘額: ')
		})
	}
})
...
mintNCCUcoinButton.on('click', async function () {
	...
	$.post('/mintCoin', {
		address: bankAddress,
		account: nowAccount,
		value: parseInt(mintNCCUCoin.val(), 10)
	}, function (result) {
		if (result.events !== undefined) {
...
buyNCCUcoinButton.on('click', async function(){
	...
	$.post('/buyCoin', {
		address: bankAddress,
		account: nowAccount,
		value: parseInt(buyNCCUcoin.val(), 10),
	}, function (result) {
		if (result.events !== undefined) {
...
transferNCCUcoinButton.on('click', async function(){
	...
	$.post('/transferCoin', {
		...
		if (result.events !== undefined) {
			log(result.events.TransferCoinEvent.returnValues, '轉移NCCU COIN成功')
...
// 檢查Owner
checkOwnerButton.on('click', function(){
	if (bankAddress == "") {
		return;
	}
	...
	$.get('/owner', {
		...
		$('#chekcOwner').text('Owner帳戶: '+ result);
	})
